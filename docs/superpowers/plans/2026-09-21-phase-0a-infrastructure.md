...
- Create: `apps/web/package.json`, `apps/web/tsconfig.json`, `apps/web/next.config.ts`, `apps/web/next-env.d.ts`, `apps/web/src/middleware.ts`, `apps/web/src/app/layout.tsx`, `apps/web/src/app/page.tsx`, `apps/web/public/lume-mark.png` (copy of `public/lume-mark.png`)
...
export const config = { matcher: [{ source: "/((?!_next/static|_next/image|favicon.ico|lume-mark.png).*)" }] };
```
...
If `next build` warns that `middleware` is deprecated in favour of `proxy`, rename the file to `src/proxy.ts` and the exported function to `proxy`. Behaviour stays identical.
...
[ "$(c -o /dev/null -w '%{http_code}' "$base/healthz")" = 200 ] && pass "GET /healthz 200" || die "/healthz"
ready="$(c -w '\n%{http_code}' "$base/readyz")"
[ "${ready##*$'\n'}" = 200 ] && pass "GET /readyz 200 ${ready%%$'\n'*}" || die "/readyz: $ready"
headers="$(c -D - -o /dev/null "$base/")"
for h in "strict-transport-security: max-age=63072000" "x-content-type-options: nosniff" "content-security-policy: default-src 'self'" "frame-ancestors 'none'"; do
  grep -qi "$h" <<<"$headers" && pass "header $h" || die "missing header $h"
done
grep -qi '^server:' <<<"$headers" && die "Server header leaked" || pass "no Server header"
c "$base/" | grep -q "LUME" && pass "web page renders" || die "web page"
[ "$(c -o /dev/null -w '%{http_code}' -H 'Origin: https://evil.example' "$base/api/v1/nope")" = 404 ] && pass "unknown API route 404" || die "api 404"
echo "smoke passed"
```
...
jobs:
  check:
    ...
    steps:
      ...
      - name: Tests
        run: set -a && . "$RUNNER_TEMP/test.env" && set +a && pnpm test
