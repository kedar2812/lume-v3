...
- **Toolbox image** `lumedev-toolbox`: node:22-bookworm + pnpm (corepack) + Docker CLI (for Testcontainers via the mounted `/var/run/docker.sock`) + Playwright browsers (added in 0B).
...
- **CI:** lint, typecheck and unit/integration tests (Testcontainers) run green on GitHub Actions. Images build and push to `ghcr.io/kedar2812/lume-v3/*` tagged by git SHA.
...
- **Language/tooling:** TypeScript 5 strict, ESLint (flat config) + Prettier, Vitest for unit/integration, Testcontainers (`@testcontainers/postgresql`) for DB tests.
...
- **Worker:** `apps/worker` starts pg-boss as `lume_worker`, registers `ops.backup` (cron `0 */6 * * *`) and `ops.restore-test` (cron `0 4 * * 1`) handlers that call `infra/scripts/backup.sh` / `restore-test.sh` through a `backup` sidecar container (Postgres client tools + age + rclone). The worker never needs DB-superuser rights.
